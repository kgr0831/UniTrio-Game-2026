Shader "Custom/VFXLit2D"
{
    Properties
    {
        ...
        _Alpha ("Alpha Override", Range(0, 1)) = 1.0
        ...
        [Toggle(_ADDITIVE_BLEND)] _AdditiveBlend ("Additive Blend", Float) = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Blend between unlit and lit
                half3 litColor = baseColor.rgb * lerp(half3(1,1,1), lighting, _LightInfluence);

                // Emission (HDR for Bloom)
                half3 emission = baseColor.rgb * _EmissionColor.rgb * _EmissionIntensity;
                litColor += emission;
                ...
                half finalAlpha = baseColor.a * _Alpha;
                    ...
                    return half4(litColor * finalAlpha, finalAlpha);
                #else
